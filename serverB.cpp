...
//defining the port numbers for the main server and backend server B
#define SERVER_M_PORT "23452"
#define SERVER_B_PORT "22452"

//structure to store names corresponding to b.txt
struct node
{
    char username[100];
    char time[100];
...
int main()
{
    ...
    struct addrinfo hints, *mainInfo, *servBInfo;
    int sockfd;
    ...
    getaddrinfo("127.0.0.1", SERVER_B_PORT, &hints, &servBInfo);

    sockfd = socket(servBInfo->ai_family, servBInfo->ai_socktype, servBInfo->ai_protocol);
    
    //binding the server process to the socket
    bind(sockfd, servBInfo->ai_addr, servBInfo->ai_addrlen);
    cout << "Server B is up and running using UDP on port " << SERVER_B_PORT << "." << endl;
    ...
    //sending names in backend server B to main server
    temp = head;
    while (temp != NULL)
    {
            ...
            while (recvTemp != NULL)
            {
                ...
                while (temp != NULL)
                {
                    ...
                    if (strcmp(temp->username, recvTemp->recvUsername) == 0)
                    {
                        ...
                        i = 0;
                        //printf("Time for the matched name:%s\n", temp->time);
